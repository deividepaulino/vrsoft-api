...
import { CursoAlunoResponseDto } from './dto/response.dto';
...
export class CursoAlunoController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Retorna a lista de cursos com maior número de alunos',
    type: Array,
  })
  @Get('top-cursos')
  ...
  @ApiResponse({
    status: 200,
    description: 'Lista todas as associações entre Alunos e Cursos.',
    type: CursoAlunoResponseDto, 
    isArray: true, 
  })
@Get()
@Get()
async findAll(): Promise<CursoAlunoResponseDto[]> {
  const cursoAlunos = await this.cursoAlunoService.findAll();
  return cursoAlunos.map(cursoAluno => ({
    codigo: cursoAluno.codigo,
    nomeAluno: cursoAluno.codigo_aluno?.nome ?? 'Aluno não encontrado',
    nomeCurso: cursoAluno.codigo_curso?.descricao ?? 'Não matriculado',
  }));
